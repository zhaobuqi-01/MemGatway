...
func (lbr *loadBalance) GetLoadBalancer(service *dao.ServiceDetail) (load_balance.LoadBalance, error) {
	...
	if service.Info.LoadType == LoadTypeTCP || service.Info.LoadType == LoadTypeGRPC {
		schema = ""
	}
	ipList := SplitStringByComma(service.LoadBalance.IpList)
	weightList := SplitStringByComma(service.LoadBalance.WeightList)
...
func (t *transport) GetTransportor(service *dao.ServiceDetail) (*http.Transport, error) {
